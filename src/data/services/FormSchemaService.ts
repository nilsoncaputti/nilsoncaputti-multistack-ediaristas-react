...
export const FormSchemaService = {
    ...
    address() {
        return yup.object().shape({
            endereco: yup.object().shape({cep: yup.string()
                .test('cep', 'CEP inválido', (value) => ValidationService.cep(value)),
                    estado: yup.string(),
                    cidade: yup.string(),
                    bairro: yup.string(),
    ...
    newContact() {
        return yup.object().shape({
            usuario: yup.object().shape({
                ...
                password_confirmation: yup.string().min(5, 'Senha muito curta').oneOf(
                    [yup.ref('password'), null], 'As senhas não estão iguais'
                ),
            }),
        }).defined();
    },
    ...
    payment() {
        return yup.object().shape({
            pagamento: yup.object().shape({
                numero_cartao: yup.string().test('card_number', 'Número de cartão inválido', (value) => {
            ...
            nome_cartao: yup.string(), validade: yup.string().test(
                'card_expiration_date',
    ...
    detalhesServico() {
        return yup.object().shape({
            ...
            hora_inicio: yup
                .string()
                .test('horaValida', 'Digite uma hora válida', (value) => 
            ...
            hora_termino: yup
                ...
                .test(
                    'hora_Termino',
                    'O serviço não pode se encerrar após as 22:00',
                    (value) => {
